...
```{r population-predictions}

nd <- crossing(Precip = seq(min(deguines_c$Precip),max(deguines_c$Precip), length.out = 50),
         Plant_biomass = seq(min(deguines_c$Plant_biomass),max(deguines_c$Plant_biomass), length.out = 50),
         plot_id = C(factor(1:30),"contr.sum"))

augment(szmodel, newdata=nd)
```
